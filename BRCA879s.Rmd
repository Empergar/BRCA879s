...
**BRCA 879s** -> ExprMX 20501g x 879s
              -> PhenoDT - OS
                         - DFS

Saber leerlo en R y hacer los boxplots correspondientes, 
                          la disribucion de los datos, 
...
biomarcadores -> ERS PGR HER2 (genes)
                 ERS-IHC, PGR-IHC, HER2-IHC (prot IHC) parcial
stage
...
if(areEqual) {
  ...
  IDC.BasalLumA <- subset(IDC, PAM50plus %in% c("Basal", "LumA"))
      ...
      # Extraemos los IDs de las muestras de interés
...
mx.phenoData.reduced <- mx.IDC.BasalLumA[selectedSamples,] 
```
...
```{r}
mx.mExpr244s.reduced.log <- log2(mx.expr244s.reduced + 1)
mx.expr244s.log <- log2(mx.expr244s + 1)
plot(density(mx.expr244s.log))

# Boxplot
boxplot(mx.mExpr244s.reduced.log , main="Boxplot de Datos log2 (10%)", ylab="Valores", col="floralwhite")
```
...
mx.mExpr244s.sin1 <- mx.expr244s[rowSums(mx.expr244s) > 1, ]

# Calculamos la logaritmica de nuevo 
mx.mExpr244s.sin1.log <- log2(mx.expr244s.sin1+ 1)

# Verificar el tamaño de la matriz después de la filtración
dim(mExpr879s)
dim(mx.mExpr244s.sin1)
dim(mx.expr244s.sin1)
```


```{r}
 plot(density(mx.mExpr244s.sin1.log[,1]))
```
...
El proceso de filtrado avanzado de los datos de expresión genética implicó la definición de un umbral de expresión mínima detectable basado en un criterio heurístico, seguido por la identificación de genes que superan este umbral en al menos una muestra.
Se estableció un umbral de expresión en el 30% de los cuartiles de la distribución de la expresión promedio, permitiendo una selección dirigida de genes activamente expresados para análisis posteriores. Este valor es similar al valor anterior de expresión mínima detectable.
...
genesAboveThreshold <- apply(mx.mExpr244s.sin1.log, 1, function(x) any(x > umbralExpresion))
genesAboveThreshold <- apply(mx.mExpr244s.sin1, 1, function(x) any(x > umbralExpresion))
...
genesToKeep <- rownames(mx.mExpr244s.sin1.log)[genesAboveThreshold]
...
mx.mExpr244s.filtered.log <- mx.mExpr244s.sin1.log[genesAboveThreshold, ]
mx.mExpr244s.filtered <- mx.mExpr244s.sin1[genesAboveThreshold, ]

# Resumen de los genes retenidos
summary(mx.mExpr244s.filtered.log)
summary(mx.mExpr244s.filtered)
...
# Boxplot
boxplot(mx.mExpr244s.filtered.log[, selectedSamples], main="Boxplot de Datos log2", ylab="Valores", col="floralwhite")
```

```{r}
plot(density(mx.mExpr244s.sin1.log), 
...
Posteriormente, por razones computacionales, generamos una versión reducida del objeto DGEList usando mx.mExpr879s.reduced. Este enfoque nos permite realizar análisis diferencial de expresión de manera eficiente, considerando las agrupaciones fenotípicas de las muestras.

```{r}
# Convertir los datos a un objeto DGEList para edgeR
# dge <- DGEList(counts=mx.mExpr879s, group=phenoData$PAM50plus)
dge <- DGEList(counts=mx.mExpr244s.filtered, group=as.data.frame(mx.IDC.BasalLumA)$PAM50plus)
```

Utilizamos la función filterByExpr de edgeR para filtrar genes en nuestro objeto DGEList, basándonos en la expresión y variabilidad entre los grupos definidos en grupo. Los genes seleccionados son aquellos con suficiente expresión y variabilidad, lo que mejora la relevancia para el análisis diferencial.

```{r}
# Filtrar los genes usando filterByExpr
group <- as.data.frame(mx.IDC.BasalLumA)$PAM50plus
